  ...
  const getSemesters = (data) => {
    ...
    const handleStudents = (data) =>{
        if(data && data.length > 0){
            ...
            localStorage.setItems('students', students);
...
// }, []);
      ...
      useEffect(() => {
    axios.post('https://cs418project-aalle063.onrender.com/admin')
    .then(res => {
      if(res.data.length === 0){
        console.log("No course history found for any student.");
        alert("No course history found for any student.");
        return;
    }
    handleStudents(res.data);
    console.log("Student courses found:");
    ...
    return(
        <><Header /><div style={{
                ...
                <div >
                 {students.map((semester, index) => (
        
                    <div key={index} style={{ padding: '15px', border: '1px solid #ddd', borderRadius: '8px', backgroundColor: '#f9f9f94b',marginBottom: '20px' }} >
                     <h3 style={{ textAlign: 'center' }}>Name {semester.name}</h3>
                     <p style={{ textAlign: 'center', color: '#666' }}>Term: {semester.term ? semester.date.toLocaleDateString() : 'N/A'}</p>
